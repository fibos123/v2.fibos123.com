<template>
  <div class="relative">
    <div class="py-6 sm:px-6 px-8">
      <nav class="flex items-center sm:h-10 justify-start">
        <div class="flex items-center flex-shrink-0 flex-grow sm:flex-grow-0">
          <div class="flex items-center justify-between w-full sm:w-auto">
            <router-link to="/" aria-label="Home">
              <svg
                class="h-8 w-auto sm:h-10 fill-current text-indigo-600"
                id="Capa_1"
                enable-background="new 0 0 511.819 511.819"
                height="512"
        ...
        <div class="hidden sm:block pl-4">
          <router-link
            ...
            :key="item.name"
            :to="item.path"
            class="font-medium text-gray-500 hover:text-indigo-600 transition duration-150 ease-in-out p-4"
            v-bind:class="{'text-indigo-600': 
            getFirstPath() === item.path
            }"
          >
            <i :class="item.icon" class="pr-1"></i>
            {{item.name}}
          </router-link>
        </div>
      </nav>
        ...
        >
          ...
          <div class="px-2 pt-2 pb-3">
            <router-link
              ...
              :to="item.path"
              @click="isOpen = !isOpen"
              class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-indigo-600 hover:bg-gray-50 focus:outline-none focus:text-indigo-600 focus:bg-gray-50 transition duration-150 ease-in-out"
              v-bind:class="{'mt-1': index, 'text-indigo-600': getFirstPath() === item.path}"
            >
              <i :class="item.icon" class="pr-1"></i>
              {{item.name}}
            </router-link>
...
export default {
  ...
  methods: {
    getFirstPath() {
      let path = "";
      const indexOf = this.$route.path.indexOf("/", 1);
      path = this.$route.path.substring(0, indexOf > 0 ? indexOf : 100);
      return path;
    },
  },
};
